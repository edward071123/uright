...
class MysqliDb
{
    ...
    /**
     * A convenient SELECT * function plus.
     *
     * @param string  $query User-provided query to execute.
     * @param int|array $numRows Array to define SQL limit in format Array ($count, $offset)
     *                               or only $count
     ...
     * @return array Contains the returned rows from the select query.
     */
    public function get_query($query, $numRows = null){
        $this->_query = 'SELECT ' . implode(' ', $this->_queryOptions) . ' ' .$query;
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->_stmtError = $stmt->error;
        $this->_stmtErrno = $stmt->errno;
    ...
    public function paginate ($table, $page, $fields = null) {
     ...
     /**
     * Pagination wraper to get_query()
     *
     * @access public
     * @param string  $query User-provided query to execute.(without "SELECT")
     * @param int $page Page number
     * @param array|string $fields Array or coma separated list of fields to fetch
     * @return array
     */
    public function paginate_query ($query, $page, $fields = null) {
        $offset = $this->pageLimit * ($page - 1);
        $res = $this->withTotalCount()->get_query ($query, Array ($offset, $this->pageLimit));
        $this->totalPages = ceil($this->totalCount / $this->pageLimit);
        return $res;
    }
